...
const cursorPagination = (): Resolver => {
  ...
  return (_parent, fieldArgs, cache, info) => {
    ...
    const allFields = cache.inspectFields(entityKey); // cache that contains queries
    const fieldInfos = allFields.filter(info => info.fieldName === fieldName); // filter by fieldname
    ...
    fieldInfos.forEach((fi) => {
      const key = cache.resolve(entityKey, fi.fieldKey) as string; // Get posts from query
...
const invalidateAllPosts = (cache: Cache) => {
  const allFields = cache.inspectFields('Query'); // cache that contains queries
  const fieldInfos = allFields.filter(info => info.fieldName === 'posts'); // filter by fieldname
  fieldInfos.forEach((fi) => {
      // invalidate the query and fetch it from the server
